...
public class Player {
    ...
    public void drawStatus(Canvas canvas) {
        ...
        Graphics.drawBorderString(canvas, 0xa33e11, 0xffde00, name,
                ViewManager.head.getWidth(), (int) (ViewManager.scale * 20), 3, paint);
        // 画生命值
        Graphics.drawBorderString(canvas, 0x066a14, 0x91ff1d, "HP: " + hp,
                ViewManager.head.getWidth(), (int) (ViewManager.scale * 40), 3, paint);
    }
    ...
    public void drawBullet(Canvas canvas) {
        ...
        for (int i = 0; i < bullets.size(); i++) {
            ...
            Graphics.drawMatrixImage(canvas, bitmap, 0, 0, bitmap.getWidth(),
                    bitmap.getHeight(), bullet.getDirection() == Player.DIR_LEFT ?
    ...
    public void logic() {
        ...
        // 如果还没有跳到最高点
        if (!isJumpMax) {
            setAction(getDirection() == Player.DIR_RIGHT ?
