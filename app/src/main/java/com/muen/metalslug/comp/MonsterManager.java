package com.muen.metalslug.comp;

import android.graphics.Canvas;

import com.muen.metalslug.GameView;
import com.muen.metalslug.ViewManager;
...
public class MonsterManager {
    ...
    public static final ArrayList<Monster> deadMonsters = new ArrayList<>();
    // 保存所有活着的怪物
    public static final ArrayList<Monster> livingMonsters = new ArrayList<>();

    // 随机生成、并添加怪物的方法
    public static void generateMonster() {
        if (livingMonsters.size() < 3 + Util.rand(3)) {
            // 创建新怪物
    ...
    public static void updatePosistion(int shift) {
        Monster monster;
        // 定义一个集合，保存所有将要被删除的怪物
        ArrayList<Monster> deletingMonsters = new ArrayList<>();
        // 遍历怪物集合
        for (int i = 0; i < livingMonsters.size(); i++) {
            monster = livingMonsters.get(i);
            if (monster == null) {
                continue;
            }
            // 更新怪物、怪物所有子弹的位置
            ...
            if (monster.getX() < 0) {
                deletingMonsters.add(monster);
            }
        }
        // 删除所有delList集合中所有怪物
        livingMonsters.removeAll(deletingMonsters);
        deletingMonsters.clear();
        // 遍历所有已死的怪物的集合
        for (int i = 0; i < deadMonsters.size(); i++) {
            monster = deadMonsters.get(i);
            if (monster == null) {
                continue;
            }
            // 更新怪物、怪物所有子弹的位置
        ...
        // 删除所有delList集合中所有怪物
        deadMonsters.removeAll(deletingMonsters);
        // 更新玩家控制的角色的子弹坐标
    ...
    public static void checkMonster() {
        // 获取玩家发射的所有子弹
        List<Bullet> bulletList = GameView.player.getBullets();
        if (bulletList == null) {
            bulletList = new ArrayList<>();
        }
        Monster monster;
        // 定义一个delList集合，用于保存将要死亡的怪物
        ArrayList<Monster> delList = new ArrayList<>();
        ...
        deadMonsters.addAll(delList);
        // 将已死亡的怪物（保存在delList集合中）从monsterList集合中删除
        livingMonsters.removeAll(delList);
    }
    ...
    public static void drawMonster(Canvas canvas) {
        ...
        deadMonsters.removeAll(delList);
    }
}
